...
> **Estado:** ✅ IMPLEMENTADO (C1–C7) — pendiente de desplegar migraciones 007–013 y `seed_monsters.py` desde PowerShell (ver CLAUDE.md, "Sistema de Campañas C1–C7"). Único diferido: mapas/tokens de batalla (C7, iteración futura). · **Fecha:** 2026-07-02
...
## 7. Estado de implementación (actualizado 2026-07-02)

Las siete fases están **implementadas y verificadas** (sintaxis, tests de servicios y `node --check`). Detalle por fase y pasos de despliegue en `CLAUDE.md` (sección "Sistema de Campañas — Fases C1–C7"). Resumen:

| Fase | Entregado | Migración |
|------|-----------|-----------|
| C1 | metadatos/estados/progresión de campaña, `milestone_level` | 007 |
| C2 | aventuras/arcos + misiones (quests) | 008 |
| C3 | mundo vivo: NPCs/localizaciones/facciones (+reputación) | 009 |
| C4 | bitácora (prep/cliffhanger/recap) + progresión XP/hitos | 010 |
| C5 | bestiario + encuentros + calculadora de dificultad DMG | 011 (+ `seed_monsters.py`) |
| C6 | rastreador de combate (iniciativa/HP/condiciones/concentración) | 012 |
| C7 | arcos/giros + guía de recompensas + visibilidad DM transversal | 013 |

**Diferido (fuera de alcance):** mapas/tokens de batalla (canvas táctico, guía §8) — mayor esfuerzo de UI, propuesto como iteración futura.

**Despliegue:** ejecutar las migraciones 007→013 en orden y `seed_monsters.py` desde PowerShell, luego commit/push (comandos exactos en cada fase de `CLAUDE.md`).
